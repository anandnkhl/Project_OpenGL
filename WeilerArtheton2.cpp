...
int vertices=5,x,y,click = 0, i = 0,intersection=0;
float t1=0.0f,t2=1.0f;
int x_inter[15], y_inter[15];
...
void clip(int lineno){
	...
	if (t1 < t2)
	{
		...
		xc2 = x1 + deltax * t2;
		yc2 = y1 + deltay * t2;
		x_inter[intersection] = xc1;
		y_inter[intersection] = yc1;
		intersection++;
		x_inter[intersection] = xc2;
		y_inter[intersection] = yc2;
		intersection++;
		//cout<<"deltax:"<<deltax<<endl;
		//cout<<"deltay:"<<deltay<<endl;
		glBegin(GL_LINES);
...
void keyboard( unsigned char key, int x, int y )
{
	if (key == 32) { 
		...
		for(int k = 0; k < vertices; k++){	//
			clip(k);
			cout<<x_inter[k]<<","<<y_inter[k]<<endl;
			cout<<"I am running in iteration number:"<< k << "\n";
		}
		click = 0;
		for(int k1=0; k1<intersection; k1++){
			glBegin(GL_LINES);
			glColor4f(0.0,1.0,0.0,1.0);
			glVertex2f(x_inter[k1],y_inter[k1]);
			glVertex2f(x_inter[k1+1],y_inter[k1+1]);
			glEnd();
			glFlush();
		}
	}
